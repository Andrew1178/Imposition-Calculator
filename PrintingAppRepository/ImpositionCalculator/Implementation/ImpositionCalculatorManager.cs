    ...
    public class ImpositionCalculatorManager : IImpositionCalculatorManager {
        ...
        public Option ReturnFinalOptionOne(PropertiesToCalculateOptionsWith props) {
            ...
            if (props.SignatureSizeWidth > 0) {
                option.Around = (int)Math.Floor((props.SheetSizeAround - (props.Gripper + props.TailMargin)) / props.SignatureSizeWidth);
            }
            else {
                option.Around = (int)Math.Floor((props.SheetSizeAround - props.Gripper - props.TailMargin) / (props.PageSizeWidth + props.Bleeds));
            }

            if (props.SignatureSizeLength > 0) {
                option.Across = (int)Math.Floor((props.SheetSizeAcross - props.SideMargin) / props.SignatureSizeLength);
            }
            else {
                option.Across = (int)Math.Floor((props.SheetSizeAcross - props.SideMargin) / (props.PageSizeLength + props.Bleeds));
            }

            option.Out = option.Around * option.Across;

            if (props.SignatureSizeWidth > 0) {
                option.Area = (float)Math.Round((props.SignatureSizeWidth * option.Around) * (props.SignatureSizeLength * option.Across), 2);
            }
            else {
                option.Area = (float)Math.Round(((props.PageSizeWidth + props.Bleeds) * option.Around) * ((props.PageSizeLength + props.Bleeds) * option.Across), 2);
            }
        ...
        public Option ReturnFinalOptionTwo(PropertiesToCalculateOptionsWith props) {
            ...
            if (props.SignatureSizeWidth > 0) {
                option.Across = (int)Math.Floor(((props.SheetSizeAcross - props.SideMargin) / props.SignatureSizeWidth));
            }
            else {
                option.Across = (int)Math.Floor((props.SheetSizeAcross - props.SideMargin) / (props.PageSizeWidth + props.Bleeds));
            }

            if (props.SignatureSizeLength > 0) {
                option.Around = (int)Math.Floor((props.SheetSizeAround - props.Gripper - props.TailMargin) / props.SignatureSizeLength);
            }
            else {
                option.Around = (int)Math.Floor((props.SheetSizeAround - props.Gripper - props.TailMargin) / (props.PageSizeLength + props.Bleeds));
            }

            option.Out = option.Across * option.Around;

            if (props.SignatureSizeWidth > 0) {
                option.Area = (float)Math.Round((props.SignatureSizeWidth * option.Across) * (props.SignatureSizeLength * option.Around), 2);
            }
            else {
                option.Area = (float)Math.Round(((props.PageSizeWidth + props.Bleeds) * option.Across) * ((props.PageSizeLength + props.Bleeds) * option.Around), 2);
            }
        ...
        /// <summary>
        /// The default is set to the 2pg one, this is set in the model when null is passed through as the multipliers.
        /// If the pages per signature is either a right angle or right angle album, then it must be set manually as 
        /// there is no correlation between the lengths.
        /// If the pages per signature is a tear apart one, the multipliers can be calculated as it is the page / 4
        /// If the pages are parallel or standard, then the default multipliers will work fine.
        /// </summary>
        ...
        public float ReturnSignatureSizeLength(SignatureSizeLength lengthParamaters) {
            if (lengthParamaters.PagesPerSignature.Contains("right angle")) {
                ...
                int amountOfPages = int.Parse(Regex.Match(lengthParamaters.PagesPerSignature, "\\d+").Value);
        ...
        public float ReturnSignatureSizeWidth(SignatureSizeWidth widthParameters) {
            int amountOfPages = int.Parse(Regex.Match(widthParameters.PagesPerSignature, "\\d+").Value);
